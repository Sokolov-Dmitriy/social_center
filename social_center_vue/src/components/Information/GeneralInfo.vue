<template>
  <div>
    <template-info v-bind:url="'generalInformation'" v-bind:header="'1.1 Общая информация'"
                   v-bind:identifier="id" v-bind:identifier_field="'client'" @addInfo="addInfo" ref="template"
      ...
      <validation-observer ref="observer" v-slot="{ handleSubmit }">
        <b-form @submit.stop.prevent="handleSubmit(save)" @reset="notSave" class="my-form">
          <b-form-group label-cols-sm="3" :label="labels[key]" label-align-sm="right"
            ...
            <validation-provider :rules="{required:false}" v-slot="validationContext" v-if="choices[key]">
            ...
            <validation-provider :rules="{required:false}" v-slot="validationContext" v-if="key==='dod'">
              <b-form-input type="date" v-model="items[key]" :value="items[key]"
                            :state="getValidationState(validationContext)"/>
            </validation-provider>
            <validation-provider :rules="{required:false}" v-slot="validationContext" v-if="key==='age'">
            ...
            <validation-provider :rules="{required: false}" v-slot="validationContext"
                                 v-if="key==='aboutNoWork' || key==='aboutWork' ">
