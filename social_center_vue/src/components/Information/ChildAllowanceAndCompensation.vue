  ...
  export default {
    ...
    created() {
      if (this.$route.params.id !== undefined)
        sessionStorage.setItem('id_economic_condition', this.$route.params.id);
      this.id = sessionStorage.getItem('id_economic_condition');
    },
    methods: {
      addInfo() {
        this.add = true;
        $.ajax({
          ...
          type: "GET",
          data: {model: 'ChildAllowanceAndCompensation'},
          success: (response) => {
      ...
      save() {
        this.items['economic_condition'] = parseInt(sessionStorage.getItem('id_economic_condition'));
        if (this.$refs.template.edit !== '')
