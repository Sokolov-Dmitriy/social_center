...
class TypologicalGroup(models.Model):
    ...
    GROUP = [
        ...
        (2, 'Группа 2.1 «Молодые женщины с социально-бытовыми проблемами в период неактивного употребления ПАВ»'),
        (3, 'Группа 2.2 «Женщины с длительным стажем употребления ПАВ и сопутствующими социально-медицинскими '
            'проблемами»')
    ]
    test = models.OneToOneField(TestBoyko, on_delete=models.CASCADE)
    client = models.ForeignKey(Client, on_delete=models.CASCADE)
